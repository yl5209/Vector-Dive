...
public class Edge : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        lineRenderer_dynamic.SetPositions(Nodes_v3.ToArray());
    ...
    void UpdateNodes(float samples)
    {
        ...
        for (int i = 0; i < density; i++)
        {
            ...
            Nodes_v3[i] = new Vector3(Mathf.Cos(angle * Mathf.Deg2Rad * i) * offset, Mathf.Sin(angle * Mathf.Deg2Rad * i) * offset, 0f);
            Nodes_v2[i] = new Vector3(Mathf.Cos(angle * Mathf.Deg2Rad * i) * offset, Mathf.Sin(angle * Mathf.Deg2Rad * i) * offset);
        }
